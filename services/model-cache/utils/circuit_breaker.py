"""
Model Cache Service - Circuit Breaker Implementation
...
class CircuitBreakerConfig:
    ...
    name: str = "model_cache"           # Circuit breaker name for logging
...
class CircuitBreaker:
    ...
    def __init__(self, config: CircuitBreakerConfig):
        ...
        logger.info(f"🔌 [MODEL_CACHE_CIRCUIT] Initialized circuit breaker '{config.name}'")
    ...
    async def call(self, func: Callable, *args, **kwargs) -> Any:
        ...
        async with self._lock:
            ...
            if self.state == CircuitState.OPEN:
                raise CircuitBreakerOpenException(
                    f"Model Cache circuit breaker '{self.config.name}' is OPEN. "
        ...
        try:
            ...
            raise CircuitBreakerTimeoutException(
                f"Model Cache circuit breaker '{self.config.name}' request timed out after {self.config.timeout}s"
            )
            ...
            if isinstance(e, self.config.expected_exception):
                ...
                logger.warning(f"⚠️ [MODEL_CACHE_CIRCUIT] Unexpected exception in '{self.config.name}': {e}")
    ...
    def _open_circuit(self):
        ...
        logger.warning(f"🔴 [MODEL_CACHE_CIRCUIT] Circuit breaker '{self.config.name}' OPENED after {self.failure_count} failures")
    
    def _half_open_circuit(self):
        """Half-open the circuit"""
        self.state = CircuitState.HALF_OPEN
        self.success_count = 0
        logger.info(f"🟡 [MODEL_CACHE_CIRCUIT] Circuit breaker '{self.config.name}' HALF-OPENED for testing")
    ...
    async def _record_success(self):
        ...
        async with self._lock:
            ...
            if self.state == CircuitState.HALF_OPEN and self.success_count >= self.config.success_threshold:
                self.state = CircuitState.CLOSED
                logger.info(f"🟢 [MODEL_CACHE_CIRCUIT] Circuit breaker '{self.config.name}' CLOSED after {self.success_count} successes")
    ...
    async def _record_failure(self, exception: Exception):
        ...
        async with self._lock:
            ...
            logger.debug(f"❌ [MODEL_CACHE_CIRCUIT] Failure #{self.failure_count} in '{self.config.name}': {exception}")
    ...
    def reset(self):
        ...
        self.last_failure_time = None
        self.last_success_time = None
        logger.info(f"🔄 [MODEL_CACHE_CIRCUIT] Circuit breaker '{self.config.name}' RESET")
...
def get_minio_breaker() -> CircuitBreaker:
    """Get circuit breaker for MinIO operations"""
    config = CircuitBreakerConfig(
        name="model_cache_minio",
        failure_threshold=3,
        recovery_timeout=60,
        success_threshold=2,
        timeout=30.0,
...
def get_external_api_breaker(service_name: str) -> CircuitBreaker:
    ...
    config = CircuitBreakerConfig(
        name=f"model_cache_external_{service_name}",
        failure_threshold=5,
