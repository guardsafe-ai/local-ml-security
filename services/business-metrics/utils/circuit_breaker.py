"""
Business Metrics Service - Circuit Breaker
...
class CircuitBreakerConfig:
    ...
    name: str = "business_metrics"      # Circuit breaker name for logging
...
class CircuitBreaker:
    ...
    def __init__(self, config: CircuitBreakerConfig):
        ...
        logger.info(f"🔌 [BUSINESS_METRICS_CIRCUIT] Initialized circuit breaker '{config.name}'")
    ...
    async def call(self, func: Callable, *args, **kwargs) -> Any:
        ...
        async with self._lock:
            ...
            if self.state == CircuitState.OPEN:
                raise CircuitBreakerOpenException(
                    f"Business Metrics circuit breaker '{self.config.name}' is OPEN. "
        ...
        try:
            ...
            raise CircuitBreakerTimeoutException(
                f"Business Metrics circuit breaker '{self.config.name}' request timed out after {self.config.timeout}s"
            )
            ...
            if isinstance(e, self.config.expected_exception):
                ...
                logger.warning(f"⚠️ [BUSINESS_METRICS_CIRCUIT] Unexpected exception in '{self.config.name}': {e}")
    ...
    def _open_circuit(self):
        ...
        logger.warning(f"🔴 [BUSINESS_METRICS_CIRCUIT] Circuit breaker '{self.config.name}' OPENED after {self.failure_count} failures")
    
    def _half_open_circuit(self):
        """Half-open the circuit"""
        self.state = CircuitState.HALF_OPEN
        self.success_count = 0
        logger.info(f"🟡 [BUSINESS_METRICS_CIRCUIT] Circuit breaker '{self.config.name}' HALF-OPENED for testing")
    ...
    async def _record_success(self):
        ...
        if self.state == CircuitState.HALF_OPEN and self.success_count >= self.config.success_threshold:
            self.state = CircuitState.CLOSED
            self.failure_count = 0
            logger.info(f"🟢 [BUSINESS_METRICS_CIRCUIT] Circuit breaker '{self.config.name}' CLOSED after {self.success_count} successes")
    
    async def _record_failure(self, exception: Exception):
        """Record a failed operation"""
        self.failure_count += 1
        self.last_failure_time = time.time()
        logger.warning(f"❌ [BUSINESS_METRICS_CIRCUIT] Circuit breaker '{self.config.name}' failure #{self.failure_count}: {exception}")
    ...
    def reset(self):
        ...
        self.last_failure_time = None
        self.last_success_time = None
        logger.info(f"🔄 [BUSINESS_METRICS_CIRCUIT] Circuit breaker '{self.config.name}' RESET")
...
# Pre-configured circuit breakers for Business Metrics service
def get_database_breaker() -> CircuitBreaker:
    """Get circuit breaker for database operations"""
    config = CircuitBreakerConfig(
        name="business_metrics_database",
        failure_threshold=3,
        recovery_timeout=30,
        success_threshold=2,
        timeout=10.0,
...
def get_external_api_breaker(service_name: str) -> CircuitBreaker:
    ...
    config = CircuitBreakerConfig(
        name=f"business_metrics_external_{service_name}",
        failure_threshold=5,
